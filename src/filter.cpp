/*
 * Copyright (C) 2020 Annika Seidel <annika.seidel@mpibpc.mpg.de>
 *
 */
...
unsigned int windowsizeGlobal;
int filterChimerics(CountProfile &countprofile, FILE *resultFile)
{

    std::vector<unsigned int> dropPositions = countprofile.getDropPointsInMaximzedProfile();

   /*if (!dropPositions.empty()) {
        std::cerr << countprofile.getSeqName() << ": ";
        for (size_t idx=0; idx<dropPositions.size(); idx++)
            std::cerr << dropPositions[idx] << ", ";
        std::cerr << endl;
    }*/

    //countprofile.showProfile(resultFile);

    bool checkPoint = countprofile.checkForRiseAndDropPoints(dropPositions, 5);
    if(checkPoint)
    {
...
int filter(int argc, const char **argv, const ToolInfo* tool)
{
    Options &opt = Options::getInstance();
    opt.parseOptions(argc, argv, *tool);
    printf("windowsize: %d\n", opt.windowsize);
    windowsizeGlobal=opt.windowsize;
    printf("seqFile: %s\n", opt.seqFile.c_str());
    ...
    processSeqFile(seqFile, resultFilename, lookuptable, translator,filterChimerics);
    delete lookuptable;
    //}
